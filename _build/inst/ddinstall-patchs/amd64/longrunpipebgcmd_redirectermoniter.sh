...
postdd(){
            ...
            [ "$type" = ext4 -o "$type" = xfs -o "$type" = btrfs ] && {
              ...
              [ $staticnetinfo != '' -a $ISDHCP != '1' ] && [ -f $therealospath/etc/network/interfaces ] && grep -q "iface.*inet" $therealospath/etc/network/interfaces && sed -i "s/iface eth0 inet dhcp/iface eth0 inet static\n  address $IP\n  netmask $MASK\n  gateway $GATE/g" $therealospath/etc/network/interfaces || printf "auto eth0\niface eth0 inet static\n  address $IP\n  netmask $MASK\n  gateway $GATE" >> $therealospath/etc/network/interfaces
...
exit 0\n" >> "$therealospath"/etc/init.d/rc.local
              ...
              [ "$CMDSTR" != '' -a "$CMDSTR_ORI" != '' ] && echo '#!/bin/bash' > $therealospath/etc/rc.local && printf "%b" "$CMDSTR_ORI""\n\n" >> $therealospath/etc/rc.local && chmod +x $therealospath/etc/rc.local
              ( umount osmnt )
            }
            [ "$layout" = "dsm" ] && {
              mkdir thebootpart
